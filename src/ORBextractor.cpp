...
namespace ORB_SLAM2
{
    ...
    static float IC_Angle(const Mat& image, Point2f pt, const vector<int>& umax)
    {
        int m_01 = 0; int m_10 = 0;
        ...
        for (int v = 1; v <= HALF_PATCH_SIZE; ++v)
        {
            ...
            for (int u = -d; u <= d; ++u)
            {
                int v_plus = center[u + v * step];
                int v_minus = center[u - v * step];
...
#undef GET_VALUE
    ...
    {
        descriptors = Mat::zeros((int)keypoints.size(), 32, CV_8U);
    ...
    ORBextractor::ORBextractor(int nFeatures, double fScaleFactor, int nLevels,
        int fIniThFAST, int fMinThFAST) :nfeatures(nFeatures), scaleFactor(fScaleFactor),
        nlevels(nLevels), iniThFAST(fIniThFAST), minThFAST(fMinThFAST)
    {
        ...
        for (int i = 1; i < nlevels; ++i)
        {
            ...
            mvLevelSigma2[i] = mvLevelSigma2[i - 1] * scaleFactor;
        }
        ...
        // need to analize again. 
        umax.resize(HALF_PATCH_SIZE + 1);
        int v, v0, vmax = cvFloor(HALF_PATCH_SIZE * sqrt(2.0f) / 2 + 1);
    ...
    void ORBextractor::operator()( InputArray _image, InputArray _mask,  std::vector<cv::KeyPoint> &_keyPoints, OutputArray _descriptors)
    {
    ...
    void ORBextractor::ComputeKeyPointsOctree(vector<vector<KeyPoint>>& _allKeypoints)
    {
        _allKeypoints.resize(nlevels);
        const int W = 30;
        ...
        for (int level = 0; level < nlevels; ++level)
        {
            computeOrientation(mvImagePyramid[level], _allKeypoints[level], umax);
        }
    }
